const BattleAbilities = [{
	...
	},
	{
		...
		'onSourceFaint' (target, source, effect) {
			if (effect && effect.effectType === 'Move' && source.template.speciesid === 'greninja' && source.hp && !source.transformed && source.side.foe.pokemonLeft) {
				...
				const template = this.getTemplate('Greninja-Ash');
	...
	},
	{
		...
		'onAnyBasePower' (basePower, source, target, move) {
			if (target === source || move.category === 'Status' || move.type !== 'Dark' || move.auraBoost) {
				return;
			}
		...
		'onEnd' (pokemon) {
			...
			for (let i = 0; i < this.sides.length; i++) {
				for (let j = 0; j < this.sides[i].active.length; j++) {
					const target = this.sides[i].active[j];
	...
	},
	{
		...
		'onUpdate' (pokemon) {
			if (['mimikyu', 'mimikyutotem'].includes(pokemon.template.speciesid) && this.effectData.busted) {
				const template = this.getTemplate(pokemon.template.speciesid === 'mimikyutotem' ? 'Mimikyu-Busted-Totem' : 'Mimikyu-Busted');
	...
	},
	{
		...
		'onStart' (pokemon) {
			...
			for (let i = 0; i < foeactive.length; i++) {
				if (!foeactive[i] || foeactive[i].fainted) {
					continue;
				}
	...
	},
	{
		...
		'onStart' (source) {
			for (let i = 0; i < this.queue.length; i++) {
				if (this.queue[i].choice === 'runPrimal' && this.queue[i].pokemon === source && source.template.speciesid === 'kyogre') {
					return;
				}
				if (this.queue[i].choice !== 'runSwitch' && this.queue[i].choice !== 'runPrimal') {
					break;
				}
			}
	...
	},
	{
		...
		'onStart' (source) {
			for (let i = 0; i < this.queue.length; i++) {
				if (this.queue[i].choice === 'runPrimal' && this.queue[i].pokemon === source && source.template.speciesid === 'groudon') {
					return;
				}
				if (this.queue[i].choice !== 'runSwitch' && this.queue[i].choice !== 'runPrimal') {
					break;
	...
	},
	{
		...
		'onAnyBasePower' (basePower, source, target, move) {
			if (target === source || move.category === 'Status' || move.type !== 'Fairy' || move.auraBoost) {
				return;
			}
			move.auraBoost = move.hasAuraBreak ? 0x0C00 : 0x1547;
	...
	},
	{
		...
		'onEnd' (pokemon) {
			if (pokemon.illusion) {
				...
				pokemon.illusion = null;
				const details = pokemon.template.species + (pokemon.level === 100 ? '' : `, L${pokemon.level}`) + (pokemon.gender === '' ? '' : `, ${pokemon.gender}`) + (pokemon.set.shiny ? ', shiny' : '');
	...
	},
	{
		...
		'onStart' (pokemon) {
			if (this.activeMove && this.activeMove.id === 'skillswap') {
				return;
			}
	...
	},
	{
		...
		'onStart' (pokemon) {
			...
			for (let i = 0; i < foeactive.length; i++) {
				if (!foeactive[i] || !this.isAdjacent(foeactive[i], pokemon)) {
					continue;
				}
				if (!activated) {
	...
	},
	{
		...
		'onAfterDamage' (damage, target, source, move) {
			if (source && source !== target && move && move.flags.contact && source.ability !== 'mummy') {
				const oldAbility = source.setAbility('mummy', source);
	...
	},
	{
		...
		'onCheckShow' (pokemon) {
			...
			for (let i = 0; i < active.length; i++) {
				...
				const template = this.getTemplate(curPoke.species);
	...
	},
	{
		...
		'onResidual' (pokemon) {
			...
			const randomTarget = pickupTargets[this.random(pickupTargets.length)];
	...
	},
	{
		...
		'onResidual' (pokemon) {
			...
			const template = this.getTemplate('Zygarde-Complete');
